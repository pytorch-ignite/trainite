...
class StringReverseDataset(Dataset):
    def __init__(
        ...
        for _ in range(size):
            ...
            # Prepare seq2seq sequences
            encoder_input = torch.cat([bos_t, seq, eos_t])
            decoder_input = torch.cat([bos_t, reversed_seq])
            decoder_target = torch.cat([reversed_seq, eos_t])

            self.encoder_input_ids.append(encoder_input)
            self.decoder_input_ids.append(decoder_input)
            self.decoder_labels.append(decoder_target)
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        return {
            ...
            "decoder_labels": self.decoder_labels[index],
        }
...
def collate_fn(batch: list[dict[str, torch.Tensor]]) -> dict[str, torch.Tensor]:
    ...
    return {
        ...
        "decoder_labels": padded_dec_labels,
    }
